...
export default function SessionControls() {
  ...
  const handleStartNewDraft = async () => {
    ...
    if (sortedTeams.length === 0)
      return alert('Add teams first before starting a draft.');
  ...
  const currentTurnTeam = draftSession.currentTurnTeamId
    ? (sortedTeams.find((team) => team.id === draftSession.currentTurnTeamId) ??
      null)
    : null;
    ...
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 flex flex-col gap-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Control the global flow of the event
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
        <div
          className={`w-3 h-3 rounded-full flex-shrink-0 ${status === 'active' ? 'bg-green-500 animate-pulse' : status === 'paused' ? 'bg-yellow-400' : status === 'ended' ? 'bg-red-400' : 'bg-gray-400'}`}
        />
        <div>
          ...
          {(draftSession.draftRound || 0) > 0 &&
            status !== 'idle' &&
            status !== 'ended' && (
              <p className="text-xs text-gray-500">
                Round #{draftSession.draftRound}
              </p>
            )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {isDraftRunning && (
          <button
            ...
            title={
              !canGoToPreviousTurn
                ? 'Previous turn is unavailable when current turn is the first team.'
                : undefined
            }
        ...
        {isDraftRunning && (
          <button
            ...
            title={
              !canSkipCurrentTurn
                ? 'Skip is available only when draft is active with a valid current team and at least 2 teams.'
                : undefined
            }
      ...
      {showEndWarning && (
        <div className="bg-amber-50 border border-amber-300 rounded-xl p-4">
          <p className="font-bold text-amber-800 mb-1">
            ⚠ Warning: Teams Haven&apos;t Drafted
          </p>
          <p className="text-sm text-amber-700 mb-3">
            {teamsWithNoPicks.length} team(s) haven&apos;t made any picks yet:{' '}
            <strong>{teamsWithNoPicks.map((t) => t.name).join(', ')}</strong>.
            Are you sure you want to force-end the draft?
          </p>
          <div className="flex gap-3">
            <button
              onClick={() => handleEndDraft(true)}
              className="px-4 py-2 bg-red-600 text-white text-sm rounded-lg font-bold hover:bg-red-700"
            >
              Force End Draft
            </button>
            <button
              onClick={() => setShowEndWarning(false)}
              className="px-4 py-2 bg-gray-200 text-gray-700 text-sm rounded-lg font-bold hover:bg-gray-300"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-100 pt-4">
        <div className="p-4 border border-gray-200 rounded-xl">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Restricts what tabs are visible and selectable.
          </p>
        </div>
        ...
        <div className="p-4 border border-gray-200 rounded-xl">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Forces teams to draft from this category right now.
          </p>
        </div>
      </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {isDraftRunning && (
                      <span
                        className={`inline-block px-2 py-1 rounded border text-[10px] uppercase tracking-wider font-black ${badgeColor}`}
                      >
                        {statusText}
                      </span>
                    )}
