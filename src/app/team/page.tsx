  ...
  if (!loggedInTeam) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
        ...
        >
          <h2 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600 mb-6 text-center">
            Team Login
          </h2>
          ...
          <div className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-blue-600 text-white font-bold p-3 rounded-lg shadow-lg shadow-blue-200 hover:bg-blue-700 transition"
            >
              Access Draft Room
            </button>
          </div>
    ...
    <div className="min-h-screen bg-gray-100 p-4 flex flex-col h-screen">
      ...
      <div className="bg-white shadow-sm rounded-xl p-4 mb-4 flex justify-between items-center border border-gray-100 flex-shrink-0">
        <div>
          <h1 className="text-2xl font-black text-gray-900">
            {liveTeamData?.name} Hub
          </h1>
          <p className="text-sm font-semibold text-gray-500">
            Your Serial: {liveTeamData?.serialNumber}
          </p>
        </div>
        <div className="flex gap-6 text-right items-center">
          <div className="hidden md:block">
            <p className="text-xs uppercase text-gray-500 font-bold mb-1 tracking-wider">
              Draft Status
            </p>
            {draftSession?.isActive ? (
              draftSession?.currentTurnTeamId === liveTeamData?.id ? (
                <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-bold animate-pulse inline-block">
                  YOUR TURN TO DRAFT
                </span>
              ) : (
      ...
      <div className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-4">
        <div className="lg:col-span-3 h-full overflow-hidden">
          <DraftOrderList
            teams={teams}
            activeTurnTeamId={draftSession?.currentTurnTeamId}
