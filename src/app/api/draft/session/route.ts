...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (data.draftStatus !== undefined)
      updateData.draftStatus = data.draftStatus;
    if (data.allowedCategories !== undefined)
      updateData.allowedCategories = data.allowedCategories;
    if (data.activeCategory !== undefined)
      updateData.activeCategory = data.activeCategory;
    if (data.currentTurnTeamId !== undefined)
      updateData.currentTurnTeamId = data.currentTurnTeamId;
    ...
    if (data.draftStartedAt !== undefined) {
      if (data.draftStartedAt === null) {
        ...
        if (Number.isNaN(parsedDate.getTime())) {
          return NextResponse.json(
            { error: 'Invalid draftStartedAt date' },
            { status: 400 }
          );
        }
