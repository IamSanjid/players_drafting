  ...
  try {
    ...
    const result = await prisma.$transaction(
      async (tx: Prisma.TransactionClient) => {
        ...
        if (!session.isActive)
          throw new Error('Draft session is currently paused');
        ...
        if (
          session.allowedCategories !== 'Both' &&
          session.allowedCategories !== player.category
        ) {
          throw new Error(
            `Only ${session.allowedCategories} players can be drafted currently.`
          );
        }

        if (session.activeCategory !== player.category) {
          throw new Error(
            `You must pick a player from the ${session.activeCategory} category right now.`
          );
        }
        ...
        if (player.category === 'Local' && player.priceBDT != null) {
          if (team.budgetBDT < player.priceBDT)
            throw new Error('Not enough BDT budget');
          await tx.team.update({
            where: { id: team.id },
            data: { budgetBDT: team.budgetBDT - player.priceBDT },
          });
        } else if (player.category === 'Oversea' && player.priceUSD != null) {
          if (team.budgetUSD < player.priceUSD)
            throw new Error('Not enough USD budget');
          await tx.team.update({
            where: { id: team.id },
        ...
        if (allTeams.length > 0) {
          const currentIndex = allTeams.findIndex(
            (t) => t.id === session.currentTurnTeamId
          );
        ...
        const allTeamsDrafted =
          picksPerTeam.length >= allTeams.length && allTeams.length > 0;
